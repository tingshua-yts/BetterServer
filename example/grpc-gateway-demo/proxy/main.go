...
func grpcCustomMatcher(key string) (string, bool) {
	...
	glog.Info("key:" + key + "   in mapping")
	switch key {
	case "X-Custom-Header1":
		return key, true
	case "X-Custom-Header2":
		return "custom-header2aaa", true
	default:
		return key, false
	}
}
func main() {
	...
	err := gen.RegisterGreeterHandlerFromEndpoint(context.Background(), mux, "localhost:8090", []grpc.DialOption{grpc.WithInsecure()})
	if err != nil {
		log.Fatal(err)
	}
